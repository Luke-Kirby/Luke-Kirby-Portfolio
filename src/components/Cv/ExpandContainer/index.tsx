...
export default class SmoothCollapse extends React.Component<Props, State> {
  ...
  static getDerivedStateFromProps(props: Props, state: State) {
    if (props.expanded && (state.closing || state.fullyClosed)) {
      ...
      !props.expanded &&
      (state.closing || state.fullyClosed) &&
      state.height !== props.collapsedHeight
    ) {
      return {
        height: props.collapsedHeight,
        renderInner:
          state.renderInner || SmoothCollapse._visibleWhenClosed(props),
      };
    }
    return null;
  }
  ...
  render() {
    const visibleWhenClosed: any = SmoothCollapse._visibleWhenClosed(
      this.props
    );
    const {
    ...
    return (
      <div
        ...
        style={{
          ...
          overflow:
            allowOverflowWhenOpen && height === "auto" ? "visible" : "hidden",
          display: fullyClosed && !visibleWhenClosed ? "none" : "",
